...
(function() {
  var App, Clock, Import, Main, SavedNotes, Sidebar, all_ids, app, arr_eq, catZero, checkboxHandler, clear_values, days, extract_values, field_style, getType, isBlank, isChecked, load_values, main_sz, menu_style, months, render_time, side_col, states, template;
  ...
  extract_values = function(id_list) {
    var id, j, len, result, target;
    result = {};
    for (j = 0, len = id_list.length; j < len; j++) {
      id = id_list[j];
      target = '#' + id;
  ...
  load_values = function(id_list, input) {
    ...
    for (j = 0, len = id_list.length; j < len; j++) {
      ...
      if ($(target).is('select')) {
        results.push($(target).val(input[id]));
      } else {
        results.push(void 0);
      }
    }
  ...
  template.conclusions.ids = ['conclusions', 'esupport', 'heatcheck', 'hc_rad1', 'hc_rad2', 'dsat_reason', 'resolved', 'reso_rad1', 'reso_rad2'];
  ...
  Sidebar = (function() {
    ...
    Sidebar.prototype.render = function() {
      var clock, clocks, j, k, len, len1, results, state;
      clocks = [];
  ...
  SavedNotes = (function() {
    ...
    SavedNotes.prototype.template = function() {
      return "<div id='saved_notes' style='height:90vh;overflow:auto'></div>";
    };

    SavedNotes.prototype.render = function() {
      var renderItem;
      $(this.el).html(this.template());
      renderItem = function(err, value) {
        $('#saved_notes').append(template.savedNote(value));
        $("#" + value.svc_tag + "-load").click(function() {
          var loaded;
          loaded = new App('#app');
          loaded.render();
        ...
        return $("[id='" + value.svc_tag + "-del']").click(function() {
          return localforage.removeItem(value.svc_tag, (function() {
            var reload, saved;
            reload = new App('#app');
            reload.render();
      ...
      return localforage.keys(function(err, keys) {
        ...
        for (j = 0, len = keys.length; j < len; j++) {
          key = keys[j];
          results.push(localforage.getItem(key, renderItem));
        }
        return results;
      });
    };
  ...
  Import = (function() {
    ...
    Import.prototype.render = function() {
      $(this.el).html(template.JSONscreen());
      localforage.keys(function(err, keys) {
        var j, key, len, result, results;
        result = [];
        results = [];
        for (j = 0, len = keys.length; j < len; j++) {
          key = keys[j];
          results.push(localforage.getItem(key, (function(err, value) {
            result.push(JSON.stringify(value));
            return $('#json_here').val(result);
          })));
        }
        return results;
      });
      return this.importPress();
    };

    Import.prototype.importPress = function() {
      return $('#import').click(function() {
        var blob, blobs, j, len;
        blobs = JSON.parse("[ " + ($('#json_here').val()) + " ]");
        for (j = 0, len = blobs.length; j < len; j++) {
          blob = blobs[j];
          localforage.setItem(blob.svc_tag, blob);
        }
        return alert('Records saved successfully');
      });
    };
  ...
  App = (function() {
    ...
    App.prototype.render = function() {
      var clock, j, len, main, main_clocks, sidebar;
      main = new Main("#main");
      ...
      for (j = 0, len = main_clocks.length; j < len; j++) {
        clock = main_clocks[j];
        clock.render();
      }
      sidebar.render();
    ...
    App.prototype.getField = function(field, button) {
      return $(button).click(function() {
        return clipboard.copy($(field).val());
      });
    };
    ...
    App.prototype.importBtn = function() {
      return $('#more_options').click(function() {
        var impt;
    ...
    App.prototype.kbShortcut = function() {
      var keys;
      keys = {};
